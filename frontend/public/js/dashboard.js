document.addEventListener("DOMContentLoaded", function () {
	...
	if (selectAll && testCheckboxes.length) {
		selectAll.addEventListener("change", function () {
			testCheckboxes.forEach((cb) => (cb.checked = this.checked))
		})
	...
	if (enableNmap && nmapOptionsHolder) {
		enableNmap.addEventListener("change", function () {
			...
			if (this.checked) createNmapOptions()
			else nmapOptionsHolder.innerHTML = ""
		})
	}
	...
	function createNmapOptions() {
        ...
        `
		const radios = nmapOptionsHolder.querySelectorAll(
			'input[name="nmap_ports"]',
		)
			...
			r.addEventListener("change", () => {
				if (specWrap)
					specWrap.style.display =
						nmapOptionsHolder.querySelector('input[name="nmap_ports"]:checked')
							.value === "specific"
							? "block"
							: "none"
			}),
	...
	function formatDuration(ms) {
		...
		return s < 60
			? `${s.toFixed(1)} detik`
			: `${Math.floor(s / 60)} menit ${Math.floor(s % 60)} detik`
	}
	...
	async function translateResults(result, targetLang) {
		...
		if (newResult.zap_alerts && Array.isArray(newResult.zap_alerts))
			newResult.zap_alerts.forEach((a) =>
				register(a, ["description", "solution"]),
			)
		if (newResult.cves && Array.isArray(newResult.cves))
			newResult.cves.forEach((c) => register(c, ["description"]))
		if (newResult.impact_analysis)
			register(newResult.impact_analysis, ["summary", "label"])
		if (
			newResult.nmap_result &&
			newResult.nmap_result.open_ports &&
			Array.isArray(newResult.nmap_result.open_ports)
		) {
	...
	function createRateLimitBarChart(codeData) {
		...
		const backgroundColors = labels.map((code) => {
			...
			if (c >= 200 && c < 300) return "rgba(16, 185, 129, 0.7)"
			if (c >= 300 && c < 400) return "rgba(59, 130, 246, 0.7)"
			if (c === 429 || c === 403) return "rgba(239, 68, 68, 0.7)"
			if (c === 415) return "rgba(236, 72, 153, 0.7)"
			if (c >= 400 && c < 500) return "rgba(245, 158, 11, 0.7)"
			if (c >= 500) return "rgba(153, 27, 27, 0.7)"
					...
					tooltip: {
						callbacks: {
							label: function (context) {
								return ` ${context.parsed.y} Request`
							},
						},
					},
				},
	...
	async function renderScanResult(result) {
		...
		// 1. SCORE CARD (UPDATED WITH CVSS & HEALTH SCORE)
		if (result.security_score) {
			const score = result.security_score
			const grade = score.final_score || "N/A"
			let color =
				grade === "A"
					? "#10b981"
					: grade === "B"
            ...
            <div class="score-card" style="border-top: 5px solid ${color};">
                <h2 style="margin:0; color:#4b5563;">${
									grade === "N/A"
										? "Status Pengujian"
										: "Penilaian Keamanan Akhir"
								}</h2>
                <span class="final-grade" style="color:${color}">${grade}</span>
                <div style="display:flex; justify-content:center; gap:15px; margin-top:10px;">
                    <div class="badge bg-critical" style="background:#374151;">Max Risk: CVSS ${
											score.max_risk_cvss || "0.0"
										}</div>
                    <div class="badge bg-info" style="background:#374151;">System Health: ${
											score.health_score_100 || "100"
										}/100</div>
                </div>
                ${
									result.impact_analysis
										? `<div style="margin-top:15px; padding:10px; background:#f8fafc; border-radius:6px; color:#555;"><strong>Ringkasan:</strong> ${escapeHTML(
												result.impact_analysis.summary,
										  )}</div>`
										: ""
								}
            </div>`
		...
		if (result.tech !== null) {
			if (result.tech.length > 0) {
				html += `<div style="margin-bottom:15px;"><strong>Teknologi:</strong> ${result.tech
					.map(
						(t) =>
							`<span style="background:#e5e7eb; padding:3px 8px; border-radius:4px; margin-right:5px;">${escapeHTML(
								t,
							)}</span>`,
					)
					.join("")}</div>`
			...
			if (result.cves && result.cves.length > 0) {
				...
				result.cves.forEach((c) => {
					if (!grouped[c.software]) grouped[c.software] = []
					grouped[c.software].push(c)
				})
				Object.keys(grouped).forEach((software) => {
					const list = grouped[software]
					let maxScore = 0
					list.forEach((c) => {
						if (c.cvss_score > maxScore) maxScore = c.cvss_score
					})
					let badgeClass =
						maxScore >= 9
							? "bg-critical"
							...
							? "bg-medium"
							: "bg-low"
					html += `<details class="cve-software-group"><summary class="cve-summary"><span><strong>${escapeHTML(
						software,
					)}</strong></span><span class="badge ${badgeClass}">${
						list.length
					} CVE (Max ${maxScore.toFixed(
						1,
					)})</span></summary><div class="cve-list-container" style="padding:0 15px;">${list
						.map(
							(c) =>
								`<div class="cve-row"><div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:5px;"><a href="https://nvd.nist.gov/vuln/detail/${
									c.cve_id
									...
									c.cvss_score >= 9
										...
										? "bg-medium"
										: "bg-low"
								}">CVSS ${Number(c.cvss_score).toFixed(
									1,
								)}</span></div><p style="margin:0 0 8px 0; font-size:0.9em; line-height:1.4;">${escapeHTML(
									c.description,
								).replace(
									/^\[.*?\]\s*/,
									"",
								)}</p><div style="font-size:0.85em; color:#059669;"><strong>Solusi:</strong> ${linkify(
									c.solution || "",
								)}</div></div>`,
						)
						.join("")}</div></details>`
		...
		if (result.xss_results !== null) {
			...
			if (result.xss_results?.length || result.sqli_results?.length) {
				...
				if (result.xss_results?.length) {
					...
					result.xss_results.forEach(
						(r) =>
							(html += `<tr><td>${escapeHTML(
								r.endpoint,
							)}</td><td><code>${escapeHTML(
								r.payload,
							)}</code></td><td style="color:red;">${escapeHTML(
								r.result,
							)}</td></tr>`),
					)
					html += `</table>`
			...
			if (!hasInjection)
				html += `<div class="secure-box"><span class="secure-icon"></span><div><strong>Aman dari Injeksi Dasar.</strong></div></div>`
		} else {
			html += `<div class="skip-box">Test Injection tidak dijalankan.</div>`
		}
		...
		if (result.zap_alerts !== null) {
			if (
				!result.zap_alerts ||
				!Array.isArray(result.zap_alerts) ||
				result.zap_alerts.length === 0
			) {
				...
				result.zap_alerts.forEach((a) => {
					...
					if (/high/i.test(risk)) risk = "High"
					else if (/medium/i.test(risk)) risk = "Medium"
					else if (/low/i.test(risk)) risk = "Low"
					const name = a.alert
					if (!grouped[risk][name])
						grouped[risk][name] = {
				...
				;["High", "Medium", "Low", "Informational"].forEach((risk) => {
					...
					if (keys.length > 0) {
						...
						keys.forEach((name) => {
							const d = alerts[name]
							html += `<details class="alert-entry"><summary>${escapeHTML(
								name,
							)} <span style="color:#666;">(${
								d.urls.size
							} URL)</span></summary><div class="alert-content"><p><strong>Desc:</strong> ${escapeHTML(
								d.description,
							)}</p><p><strong>Sol:</strong> ${escapeHTML(
								d.solution,
							)}</p><ul class="url-list">${Array.from(d.urls)
		...
		if (result.ssl_result !== null) {
			...
			if (!ssl)
				html += `<div style="padding:10px; color:red;">Gagal SSL Scan.</div>`
			else if (ssl.error)
				html += `<div style="padding:10px; color:red;">${escapeHTML(
					ssl.error,
				)}</div>`
			else {
				html += `<h4>SSL/TLS</h4>`
				...
				if (issues.length === 0 && !ssl.certificate?.error) {
					...
					html += `<div style="background:#fef2f2; border:1px solid #fecaca; padding:10px; border-radius:6px; margin-bottom:10px;"><ul>${issues
				...
				if (ssl.certificate && !ssl.certificate.error) {
                    ...
                    <div><strong>Issuer:</strong> ${escapeHTML(
											ssl.certificate.issuer,
										)}</div>
                    <div><strong>Valid:</strong> ${
											escapeHTML(ssl.certificate.valid_from).split("T")[0]
										} s/d ${
						escapeHTML(ssl.certificate.valid_until).split("T")[0]
					}</div>
                    <div><strong>Trusted:</strong> ${
											ssl.certificate.is_trusted ? "Ya" : "Tidak"
										}</div>
                    </div>`
				}
			}
		} else {
			html += `<div class="skip-box">Test SSL tidak dijalankan.</div>`
		}

		if (result.nmap_result !== null) {
			html += `<h4>Port Scanning</h4>`
			if (
				result.nmap_result.open_ports &&
				result.nmap_result.open_ports.length > 0
			) {
				...
				result.nmap_result.open_ports.forEach((p) => {
					...
					html += `<tr><td><b>${
						p.port
					}</b></td><td><span class="badge ${cls}">${escapeHTML(
						p.state,
					)}</span></td><td>${escapeHTML(p.service)}</td><td>${escapeHTML(
						p.version || "-",
					)}</td><td><span class="badge ${riskCls}" style="font-size:0.7em;">${
						p.risk || "Info"
					}</span> <br><span style="font-size:0.85em;">${escapeHTML(
						p.advice || "",
					)}</span></td></tr>`
				})
				html += `</tbody></table>`
			} else {
				html += `<div class="secure-box"><div><strong>Tidak Ada Port Terbuka.</strong></div></div>`
			}
		} else {
			html += `<div class="skip-box">Test Nmap tidak dijalankan.</div>`
		}
		...
		if (result.ratelimit_result !== null) {
			...
			if (rl.error) {
				html += `<div class="error-box"><strong>Gagal Menjalankan Test:</strong><br>${escapeHTML(
					rl.error,
				)}</div>`
			} else {
				const isSafe =
					rl.summary &&
					(rl.summary.toUpperCase().includes("AMAN") ||
						rl.summary.toUpperCase().includes("MITIGATED"))
				const color = isSafe
					? "d1fae5"
					: rl.summary && rl.summary.toUpperCase().includes("RENTAN")
					? "fee2e2"
					: "ffedd5"

				html += `<div style="padding:15px; margin-bottom:15px; background:#${color}; border-radius:8px; font-weight:500;">${escapeHTML(
					rl.summary || "Pengujian selesai.",
				)}</div>`
		...
		if (
			result.ratelimit_result &&
			result.ratelimit_result.details &&
			result.ratelimit_result.details.codes
		) {
			setTimeout(() => {
			...
			.addEventListener("click", async function () {
				...
				try {
					...
					if (resp.status === "success") {
						...
						throw new Error(resp.error || "Gagal generate PDF")
					}
				} catch (e) {
					btn.innerText = "Gagal"
	...
	if (scanForm) {
		scanForm.addEventListener("submit", async function (e) {
			...
			progressStatus.innerHTML = "Sedang memproses scan..."
			...
			let rlLevel = "0"
			const isRlChecked = document.querySelector(
				'input[name="tests"][value="ratelimit"]',
			)?.checked
			...
			const payload = {
				...
				nmap_ports:
					document.querySelector('input[name="nmap_ports"]:checked')?.value ||
					"top1000",
				nmap_specific_ports:
					document.querySelector('input[name="nmap_specific_ports"]')?.value ||
					"",
				nmap_show_os:
					document.querySelector('input[name="nmap_show_os"]')?.checked ||
					false,
				nmap_show_service:
					document.querySelector('input[name="nmap_show_service"]')?.checked ||
					false,
				ssl_enabled:
					document.querySelector('input[name="tests"][value="ssl"]')?.checked ||
					false,
			...
			try {
				...
				let timeStr = ""
				if (elapsedMs < 60000) {
					timeStr = `${(elapsedMs / 1000).toFixed(1)} detik`
				} else {
					const mins = Math.floor(elapsedMs / 60000)
					const secs = Math.floor((elapsedMs % 60000) / 1000)
					timeStr = `${mins} menit ${secs} detik`
				}
				...
				progressStatus.innerHTML = `Test selesai dalam: <strong>${timeStr}</strong>`
				...
				progressStatus.innerHTML = `✗ Scan gagal dalam ${timeStr}: ${err.message}`
				toggleOverlay(false)
			} finally {
				if (btn) {
					btn.disabled = false
